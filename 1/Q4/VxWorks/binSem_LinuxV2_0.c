...
void fib10(void)
{
   ...
   while(!abortTest)
   {
	   ...
	   FIB_TEST(seqIterations, 1700000);
       //FIB_TEST(seqIterations, 150000);
	   //printf("\nexec FIB10 TEST\n");
       //FIB_TEST(seqIterations, 1300000);
...
void fib20(void)
{
   while(!abortTest)
   {
	   ...
	   //printf("\nexec FIB20 TEST\n");
	   //FIB_TEST(seqIterations, 3000000);
	   //FIB_TEST(seqIterations, 1300000);
	   //FIB_TEST(seqIterations, 300000);
	   FIB_TEST(seqIterations, 3400000);
...
int main (int argc, char *argv[])
{
   ...
   unsigned int cpusize = sizeof(new_mask);
...
// Setting the affinity 
	rc=sched_getaffinity(getpid(),cpusize,NULL);
	rc=sched_getaffinity(getpid(),cpusize,&cpuset);
	if (rc)
	{
			printf("Error in get sched affinity\n");
			perror(NULL);
		    exit(-1);
	}
	
	printf(" sched_getaffinity = %d, cur_mask = %08lx  size = %zu \n ", rc,cpuset,cpusize);
	
	rc = sched_setaffinity(getpid(), cpusize, &cpuset);
	
   if (rc)
	{
			printf("Error in set sched affinity\n");
			perror(NULL);
		    exit(-1);
	}
...
void *startService(void *threadid)
{
   ...
   rc = pthread_create(&threads[FIB10_SERVICE], &rt_sched_attr[FIB10_SERVICE], fib10, (void *)FIB10_SERVICE);
	...
	CPU_ZERO(&cpuset);
	CPU_SET(0, &cpuset);
	s = pthread_setaffinity_np(startService, sizeof(cpu_set_t), &cpuset);
	s = pthread_setaffinity_np(fib10, sizeof(cpu_set_t), &cpuset);
	s = pthread_setaffinity_np(fib20, sizeof(cpu_set_t), &cpuset);
	*/
   //Assigning Affinity , One CPU to all threads 
  ...
  while(!abortTest)
  {
	  ...
	  //usleep(10000); sem_post(&semF10);
	  //printf("\nIn F10 3\n");
      usleep(10000); sem_post(&semF20);//sem_post(&semF10);
