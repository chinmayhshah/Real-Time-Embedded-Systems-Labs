...
int main (int argc, char *argv[])
{
   ...
   //Assigning policy and attribute initialization for different pthreads
   ...
   print_scheduler();
    ...
    //min and max values of real time
   ...
   rt_param[START_SERVICE].sched_priority = rt_max_prio;//starts service highest priority --99
   pthread_attr_setschedparam(&rt_sched_attr[START_SERVICE], &rt_param[START_SERVICE]);

    //Creating thread 0 and calling startService
   ...
   if(pthread_join(threads[START_SERVICE], NULL) == 0)
    ...
    //Set Scheduler after all threads are created
...
void *startService(void *threadid)
{
   ...
   runInterference=intfTime;//input from user ,c command line

   rt_param[HIGH_PRIO_SERVICE].sched_priority = rt_max_prio-1;//98, as thread 0 --> 99
   pthread_attr_setschedparam(&rt_sched_attr[HIGH_PRIO_SERVICE], &rt_param[HIGH_PRIO_SERVICE]);

   //creating thread 1 and calling IDLE
   printf("Creating thread %d\n", HIGH_PRIO_SERVICE);
   ...
   rt_param[MID_PRIO_SERVICE].sched_priority = rt_max_prio-10;//89
   ...
   rt_param[LOW_PRIO_SERVICE].sched_priority = rt_max_prio-20;//79
